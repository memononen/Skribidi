 ...
 * For that reason manual rendering would look something like this:
 *
 *		// Create copy of the (clipping) mask, and create temporary layer to render to.
 *		skb_canvas_push_layer(...);
 *
 *		// Define shape to render
 *		skb_canvas_move_to(...);
 *		skb_canvas_line_to(...);
 *		skb_canvas_close(...);
 *
...
/**
 * Creates new canvas to render to.
 * The canvas is expected to be disposable. You create canvas, render some image, and dispose it.
 ...
 * @param temp_alloc temp allocator to use during rendering.
 ...
 * Shapes rendered after the push will be clipped to the mask.
 ...
 * That is, the earlier mask is the clipping mask for the rendered shape.
 * If you call the fill methods multiple times without push/pop mask, the filled area will get smaller and smaller.
 * This fill method is used when rendering to an Alpha image.
 * @param c canvas to render to
 */
